...
//        ts_filepath = ros::package::getPath(package_name_2).append("/config/ts_mini_cheetah_test.yaml");
    ...
    void create_monitors(){
        ...
        for(int i=0; i<number_of_dimensions; i++){
            ...
            } else if (dimension == "mini_cheetah_load") {
                // always initialize as unloaded for now
                current_ltl_state_[i] = "standby";
            } else {
                std::cout <<"state type " << dimension << " is not supported by mini cheetah TS" << std::endl;
            }
        }
...
int main(int argc, char** argv){
    ros::init(argc, argv, "mini_cheetah_ltl_bt");
    LTLA1Planner runner;
    ros::spin();
    return 0;
}
